  ...
  <div class="order-confirm">
      ...
      <div class="container">
        <div class="order-box">
          ...
          <div class="item-address">
            ...
            <div class="addr-list clearFloat">
              ...
              <div class="addr-info" :class="{'checked':index == checkIndex}" @click="checkIndex = index" v-for="(item,index) in list" :key="index">
                ...
                <div class="action">
                 ...
                 <!-- 增加 or 修改 -->
                  <a href="javascript:;" class="fr" @click="upAddress(item)">
    ...
    >
      <template v-slot:body>
        <!-- 添加地址的表单 -->
        <div class="edit-wrap" v-if="addSlot">

          <div class="item">
            <input type="text" class="input" placeholder="姓名" v-model="addressInfo.username">
            <input type="text" class="input" placeholder="手机号" v-model="addressInfo.Phone">
          </div>

          <div class="item">
            <select name="province" v-model="addressInfo.Province">
              <option value="北京">北京</option>
              <option value="天津">天津</option>
              <option value="河北">河北</option>
            </select>
            <select name="city" v-model="addressInfo.City">
              <option value="北京">北京</option>
              <option value="天津">天津</option>
              <option value="河北">石家庄</option>
            </select>
            <select name="district" v-model="addressInfo.District">
          ...
          <div class="item">
            <textarea name="street" v-model="addressInfo.Street"></textarea>
          </div>

          <div class="item">
            <input type="text" class="input" placeholder="邮编" v-model="addressInfo.Postcode">
          </div>
        </div>

        <p v-if="deleteSlot">您确认要删除此地址吗？</p>
      </template>
...
export default{
  ...
  data(){
    return {
      ...
      checkIndex:0,          //当前收货地址 默认选中第一个索引

      checkedItem:{},       //选中的地址信息对象
  ...
  methods:{
    ...
    orderSubmit(){
     ...
     if(!this.cartList.length){
        this.$message.error("您当前没有商品需要结算");
        return ;
     }
    ...
    upAddress(item){
        ...
        this.addressInfo.username = item.receiverName,
        this.addressInfo.Phone = item.receiverMobile
        this.addressInfo.Province = item.receiverProvince
        this.addressInfo.City = item.receiverCity,
        this.addressInfo.District = item.receiverDistrict,    //区
        this.addressInfo.Street = item.receiverAddress,         // 街道详细地址
        this.addressInfo.Postcode = item.receiverZip;   
    ...
    //删除 编辑  新增
    submitAddress(){
      let {checkedItem, userAction,addressInfo} = this;
      ...
      if(userAction==0 || userAction==1){
        ...
        if(!addressInfo.username){
              errMsg = '请输入收货人名称';
        }else if(!addressInfo.Phone||!/\d{11}/.test(addressInfo.Phone)){ 
              alert(addressInfo.Phone);
              errMsg = '请输入正确手机号码格式的';
        }else if(!addressInfo.Province){
              errMsg = '请选择身份';
        }else if(!addressInfo.City){
              errMsg = '请选择对应的城市';
        }else if(!addressInfo.District||!addressInfo.Street){
              errMsg = '请输入收货地址'
        }else if(!/\d{6}/.test(addressInfo.Postcode)){
              errMsg = '请输入邮政编码';
        }
...
//alert(errMsg);
        if(errMsg){
          ...
          params={
            ...
            receiverMobile:addressInfo.Phone,
            receiverProvince:addressInfo.Province,
            receiverCity:addressInfo.City,
            receiverDistrict:addressInfo.District,    //区
            receiverAddress:addressInfo.Street,         // 街道详细地址
            receiverZip:addressInfo.Postcode,    
        }
      }
